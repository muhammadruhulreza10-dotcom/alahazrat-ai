...
.arabic-ur-ibarath {{
    ...
    font-family: 'Scheherazade New', 'Traditional Arabic', 'Amiri', 'Noto Naskh Arabic', 'Jameel Noori Nastaleeq', sans-serif !important;
...
with st.sidebar:
    ...
    uploaded_files = st.file_uploader("এখানে PDF upload করুন (সরাসরি গুগল এআই রিড করবে)", type=["pdf"], accept_multiple_files=True)
            ...
            for uploaded_file in uploaded_files:
                if uploaded_file.name not in st.session_state["uploaded_file_names"]:
                    ...
                    try:
                        ...
                        st.error(f"আপলোড ট্রাবল: {str(upload_error)}")
                    finally:
                        if os.path.exists(temp_path):
                            os.remove(temp_path)
            st.success("✅ কিতাব সফলভাবে গুগল এআই-তে যুক্ত হয়েছে!")
            st.rerun()
...
# ---------------- CHAT INPUT ----------------
if prompt := st.chat_input("কিতাব থেকে যেকোনো প্রশ্ন বা রেফারেন্স জানতে লিখুন..."):
            ...
            try:
                ...
                if any(x in error_msg for x in ["429", "RESOURCE_EXHAUSTED", "403", "PERMISSION_DENIED"]):
                    ...
                    if next_index < len(st.secrets["GEMINI_API_KEYS"]):
                        ...
                        st.warning("⚠️ বর্তমান ফ্রি সার্ভারের কোটা শেষ হওয়ায় আপনার ২য় ব্যাকআপ সার্ভারে শিফট করা হয়েছে। অনুগ্রহ করে আর একবার প্রশ্নটি সাবমিট করুন।")
                    else:
                        st.session_state["current_key_index"] = 0
                        st.error("❌ দুঃখিত, যুক্ত করা সবকটি ফ্রি কী-এর লিমিট এই মুহূর্তের জন্য শেষ। দয়া করে কিছুক্ষণ পর চেষ্টা করুন।")
                else:
                    st.error(f"দুঃখিত, কিতাবখানা থেকে উত্তর তৈরিতে সাময়িক সমস্যা হয়েছে। এরর: {error_msg}")
